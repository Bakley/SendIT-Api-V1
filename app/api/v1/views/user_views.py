...
class SignupResource(Resource):
    ...
    # <RequestParser> Enables adding and parsing of multiple arguments in the context of a single request
    ...
    def post(self):
        ...
        # <parse_args> Parse all arguments from the provided request and return the results as a Namespace
        ...
        if not (re.match(username_format, username)):
            return {'message' : 'Invalid username, only characters allowed no intergs allowed'}, 400
        ...
        username_exists = UserModel.get_user_by_username(username=args['username'])
        email_exists = UserModel.get_user_by_email(email=args['email'])

        if username_exists or email_exists:
            return {'message': 'User already exists'}, 203
        
        user = UserModel(username=args.get('username'), email=args.get('email'),  role=args.get('role'), password=password)
        user = user.save()
    ...
    def get(self):
        ...
        if not user:
            return {'message': 'Users have not yet been registered. Please go and register'}, 404
...
class LoginResource(Resource):
    ...
    def post(self):
        ...
        if not user:
            return {'message': 'User has not yet been registered. Please go and register'}, 404
